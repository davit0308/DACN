...
class FastGA:
    ...
    def __init__(self, parameter):
        ...
        # Edge constraint validator for early-exit optimization
        if _HAS_VALIDATOR:
            self._validator = EdgeConstraintValidator(verbose=False)
            # Pre-compute which edges in current stage are impossible
            self._impossible_edges = self._find_impossible_edges()
        else:
            self._validator = None
            self._impossible_edges = set()

        self.pop = self._init_pop()
        self.bestindividual = self._select_best(self.pop)

    # ------------------------------------------------------------------
    # PRE-COMPUTE: find edges that violate type constraints
    # ------------------------------------------------------------------
    def _find_impossible_edges(self) -> set:
        """
        Scan edges in the current stage and mark those that fail
        type-level validation. Returns set of global_edge_idx values.
        """
        if not self._validator:
            return set()

        impossible = set()
        if self.stage - 1 < len(self.relation_list):
            for edge in self.relation_list[self.stage - 1]:
                src_idx = int(edge[0])
                dst_idx = int(edge[1])
                verb = edge[2]
                if src_idx < len(self.entity_list) and dst_idx < len(self.entity_list):
                    src_type = self.entity_list[src_idx]
                    dst_type = self.entity_list[dst_idx]
                    if not self._validator.validate_edge(src_type, verb, dst_type):
                        impossible.add(edge[3])

        if impossible:
            logger.info(
                f"FastGA stage {self.stage}: {len(impossible)} impossible edges "
                f"pre-flagged for early-exit"
            )
        return impossible
    ...
    def _evaluate(self, geneinfo: List[int]):
        ...
        # EARLY EXIT: if chromosome activates rules that ONLY match
        # impossible edges, skip the expensive evaluation
        if self._impossible_edges and self._validator:
            active_rules = {i for i, v in enumerate(geneinfo) if v == 1}
            if active_rules:
                rule_match_edge_dic, _ = self._match_rule(
                    self.relation_list, self.entity_list,
                    self.stage, self.stage_len_1
                )
                # Check if ALL active rules only produce impossible edge matches
                all_impossible = True
                for rule_idx in active_rules:
                    matches = rule_match_edge_dic.get(rule_idx, [])
                    for cluster in matches:
                        for edge_idx in cluster:
                            if edge_idx not in self._impossible_edges:
                    ...
                    if not all_impossible:
                        break
                if all_impossible and active_rules:
                    # Every active rule only touches impossible edges
                    relation = self.relation_list[self.stage - 1]
                    e_dic = {int(ep): [] for rel in relation for ep in [rel[0], rel[1]]}
                    r_dic = {rel[3]: [] for rel in relation}
                    zero_gene = [0] * len(geneinfo)
                    if len(self._eval_cache) < self.DUPLICATE_CACHE_SIZE:
                        self._eval_cache[gene_key] = (
                            0.0, tuple(zero_gene), dict(e_dic), dict(r_dic)
                        )
                    return 0.0, zero_gene, e_dic, r_dic
        ...
        # ── STRICT INSTANCE-LEVEL VALIDATION ──
        # Check every matched edge against EdgeConstraintValidator.
        # If ANY matched edge violates type constraints, reject the
        # entire chromosome with fitness=0. No soft penalties.
        if fitness > 0 and self._validator:
            for rel in relation:
                edge_idx = rel[3]
                if relation_arrange_dic.get(edge_idx, 0) == 1:
                    src_idx = int(rel[0])
                    dst_idx = int(rel[1])
                    verb = rel[2]
                    if src_idx < len(self.entity_list) and dst_idx < len(self.entity_list):
